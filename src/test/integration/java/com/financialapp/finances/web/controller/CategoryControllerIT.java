...
class CategoryControllerIT {
    ...
    @Test
    void getReturnsTheCategory() throws Exception {
        when(getCategory.execute(new CategoryId(1L), new UserId(42L))).thenReturn(food());
        mvc.perform(get("/api/v1/finances/categories/1").header("X-User-Id", "42"))
                .andExpect(status().isOk())
    ...
    @Test
    void updateReturnsRenamedCategory() throws Exception {
        when(updateCategory.execute(any())).thenReturn(food());
        mvc.perform(put("/api/v1/finances/categories/1").header("X-User-Id", "42")
                        .contentType(MediaType.APPLICATION_JSON).content("{\"name\":\"Food\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.name").value("Food"));
    }
    ...
    @Test
    void restoreCategoryReturnsRestoredCategory() throws Exception {
        when(restoreCategory.execute(any())).thenReturn(food());
        mvc.perform(post("/api/v1/finances/categories/1/restore").header("X-User-Id", "42"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.name").value("Food"));
    }

    @Test
    void listSubcategoriesReturnsThem() throws Exception {
        when(listSubcategories.execute(new CategoryId(10L), new UserId(42L)))
                .thenReturn(List.of(Subcategory.reconstitute(new CategoryId(11L), new CategoryName("Dining"), CategoryStatus.ACTIVE)));
        mvc.perform(get("/api/v1/finances/categories/10/subcategories").header("X-User-Id", "42"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data[0].id").value(11))
                .andExpect(jsonPath("$.data[0].name").value("Dining"));
    }

    @Test
    void createSubcategoryReturns201WithTheJustAddedSubcategory() throws Exception {
        when(createSubcategory.execute(any())).thenReturn(foodWithSubcategory(CategoryStatus.ACTIVE));
        mvc.perform(post("/api/v1/finances/categories/10/subcategories").header("X-User-Id", "42")
                        .contentType(MediaType.APPLICATION_JSON).content("{\"name\":\"Dining\"}"))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.data.id").value(11))
                .andExpect(jsonPath("$.data.name").value("Dining"));
    ...
    @Test
    void archiveSubcategoryReturnsSuccess() throws Exception {
        mvc.perform(delete("/api/v1/finances/categories/10/subcategories/11").header("X-User-Id", "42"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));
    }
}
